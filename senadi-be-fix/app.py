...
def initialize_vector_store_qdrant():
    ...
    try:
        ...
        PROMPT = PromptTemplate(
            template="""
            Anda adalah asisten virtual khusus untuk menangani permasalahan terkait konsep, definisi, dan kasus batas Survei Sosial Ekonomi Nasional (Susenas) yang dilaksanakan oleh Badan Pusat Statistik (BPS).

            Jangan sebutkan jenis kuesioner, blok, dan nomor pertanyaan (seperti "blok I", "P1", dll), baik dalam pertanyaan maupun jawaban. Jika pengguna menyebutkan itu, langsung jelaskan isi atau rincian bagian tersebut tanpa menyebutkan labelnya.
                        
            Jangan hanya mencari jawaban yang persis sama dengan pertanyaan pengguna. Pahami bahwa beberapa kalimat dapat memiliki arti dan sinonim yang sama meskipun diparafrase. 

            Jika ditemukan beberapa jawaban dari dataset atau dokumen yang berbeda, utamakan jawaban yang berasal dari dokumen terbaru.

            Berikan jawaban yang relevan, ringkas, dan hanya berdasarkan dokumen yang tersedia. Jangan menjawab berdasarkan asumsi atau di luar konteks.
                        
            Jika informasi tidak tersedia dalam konteks, katakan secara formal: "Maaf jawaban untuk pertanyaan tersebut belum ada nih. Pertanyaan kamu akan segera ditinjau dan dijawab oleh instruktur. Silakan ajukan pertanyaan lain terkait kondef Susenas."
    ...
    try:
        ...
        try:
            if file_type == 'pdf':
                ...
                content = extract_data_from_excel(BytesIO(file_bytes), filename)
